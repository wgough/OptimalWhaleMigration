...
energy_model <- function(MigrationSwimSpeeds_ms, BreedingDays, MigrationDistance_km, BodyLength_m, LungesPerDay, PercentDaysSpentFeeding, PercentIntakePerLunge, PercentFluking, Species) {
  ...
  ENetFeedGroundDay_kJ = ENetFeedSeason_kJ / FeedingGroundDays
...
modifier <- c(0.9, 0.95, 1, 1.05, 1.1)
...
dataframe_humpback = expand_grid(MigrationSwimSpeeds_ms = seq(1, 4, by = 0.1),
                                 ...
                                 PercentDaysSpentFeeding = 0.5 * modifier,
                                 PercentIntakePerLunge = 0.5 * modifier,
  ...
  mutate(model_results = pmap(
    ...
    ECostFeedSeason_kJ = map_dbl(model_results, "ECostFeedSeason_kJ"),
    EIntakeLunge = map_dbl(model_results, "EIntakeLunge"),
    EIntakeFeedSeason_kJ = map_dbl(model_results, "EIntakeFeedSeason_kJ"),
...
selected_row <- dataframe_humpback %>%
  filter(
    MigrationDistance_km == 4000,
    BreedingDays == 50,
    MigrationSwimSpeeds_ms == 2,
    BodyLength_m == 12,
    LungesPerDay == 546,
    PercentFluking == 0.55,
    PercentDaysSpentFeeding == 0.5,
    PercentIntakePerLunge == 0.5
  )
```
...
pMEP <- ggplot(filter(dataframe_combined, MigrationDistance_km != 12000)) +
  ...
  annotate("text", x=17600, y=170, label="95% Percentile of Distance", size = 16, angle="90") +
  coord_cartesian(xlim = c(3000,21000), ylim = c(0,200), expand = TRUE) +
  scale_x_continuous(breaks = seq(4000, 20000, by = 4000)) +
...
# Update the plot
pSC <- ggplot(filter(speed_averaged_dataframe, MigrationDistance_km != 12000)) +
  ...
  geom_segment(data=filter(time_chunk_dataframe, MigrationDistance_km != 12000), aes(x = OptSpeed,
                   xend = OptSpeed,
                   y = MinRatio-0.25,
                   yend = MinRatio+0.25),
               size = 4) +
  geom_pie_glyph(data=filter(time_chunk_dataframe, MigrationDistance_km != 12000), 
                 aes(x=OptSpeed, y=MinRatio, fill=TimeSlice), 
                 slices = 'TimeSlice', values = 'Days', colour = 'black', radius = 2.5) +
  annotate("text", x=0.51, y=-0.943, label="2000km", size = 16) +
  annotate("text", x=0.51, y=-0.408, label="4000km", size = 16) +
  annotate("text", x=0.51, y=0.308, label="6000km", size = 16) +
  annotate("text", x=0.615, y=1.254, label="8000km", size = 16) +
  annotate("text", x=0.76, y=1.324, label="10000km", size = 16) +
  #annotate("text", x=0.91, y=0.375, label="12000km", size = 16) +
  coord_cartesian(ylim = c(-1.7, 1.35)) +
  scale_y_continuous(breaks = seq(-1.5, 1.25, by = 0.25), labels = scales::label_number(accuracy = 0.01)) +
...
Given the potential for substantial variability in our model, we generated anchored ranges (±10%) for a series of critical variables and performed our calculations for each combination of values. These variables included total body length ($TL_{\text{body}}$; m), the number of feeding lunges performed per day ($N_{\text{lunges}}$; lunges day^-1^), and the percentage of time (in seconds) spent actively swimming (i.e., expending energy to generate propulsive tailbeats) throughout an average day ($\%_{\text{fluke}}$; s), the percentage of days spent on the feeding ground that included active foraging ($\%_{\text{feed}}$), the percentage of feeding season energy intake relative to our predicted maximum ($\%_{\text{intake}}$), and the number of days spent on the breeding ground in a given year ($T_{\text{breed}}$; days). We used our UAS-photogrammetry dataset to anchor body length at 22 m for blue whales and 12 m for humpback whales. For the daily lunge rate, we took a value of 204 lunges day^-1^ for blue whales and 546 lunges day^-1^ for humpack whales from Savoca et al. [-@savoca_etal21]. We estimated the percentage of time spent fluking at 35% (30,240 s) for blue whales and 55% (47,520 s) for humpback whales using our published CATS tag dataset and tailbeat-detection methods outlined by Gough et al. [-@gough_etal19; -@gough_etal21]. We estimated the percentage of days spent actively foraging the feeding ground at 50% to account for non-foraging behaviors (e.g., searching, socializing). Using the intake values modeled by Gough et al.[-@gough_etal22] as a theoretical kinematic and energetic maximum for a feeding lunge, we estimated our energy intake for a given season to be 50%, thus bringing us more in line with lower estimates such as those suggested AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA (In the absence of more reliable estimates for blue whales, we set the amount of time spent on the breeding ground at 50 days for both blue and humpback whales from sighting data of humpback whales in Maui Nui (Evans and van Aswegen, pers. comm.). 

In order to capture the majority of potential migrations undertaken by large cetaceans, we performed our energetic calculations for unidirectional migration distances ($D_{\text{mig}}$; km) ranging from 2,000-12,000 km.
...
We started by estimating the energetic intake from a feeding season ($E_{\text{feed}}$; kJ) of a given length ($T_{\text{feed}}$; days) by subtracting the seasonal energetic costs ($E_{\text{cost.feed}}$; kJ) from the seasonal energetic intake ($E_{\text{intake.feed}}$; kJ). We calculated the length of time spent on the feeding season:

$$
T_{\text{feed}} = 365 - T_{\text{breed}} - (T_{\text{mig}}*2) * \%_{\text{feed}} \quad (1)
$$

as the remainder of the annual cycle (365 days) after subtracting the length of the breeding season and the length of the migration: estimated through the interaction of over-ground speed and unidirectional migration distance (multiplied by two to account for the two unidirectional migrations performed in a year). We then multiplied the length of time spent on the feeding grounds by a percentage  For the energetic intake and cost of the feeding season, we used the following equations:
